...
CREATE TYPE "ai"."ingestion_task_entity_type" AS ENUM('syllabus', 'assignment', 'course', 'tabs', 'rawDoc');--> statement-breakpoint
CREATE TYPE "ai"."ingestion_task_kind" AS ENUM('course:Plan', 'course:Change', 'course:FullIngest', 'fetch:AllAssignments', 'fetch:CourseInfo', 'process:Syllabus', 'write:CourseInfo', 'process:Tabs', 'write:Syllabus');--> statement-breakpoint
...
CREATE TABLE "ai"."canvas_raw_documents" (
	...
	CONSTRAINT "canvas_raw_documents_canvas_course_id_school_id_entityType_entityId_unique" UNIQUE("canvas_course_id","school_id","entity_type","entity_id")
);
--> statement-breakpoint
...
CREATE TABLE "ai"."ingestion_runs" (
	...
	"school_id" bigint NOT NULL,
	CONSTRAINT "ingestion_runs_school_id_unique" UNIQUE("school_id")
);
--> statement-breakpoint
CREATE TABLE "ai"."ingestion_tasks" (
	...
	"updated_at" timestamp with time zone,
	"created_at" timestamp with time zone DEFAULT now() NOT NULL,
	CONSTRAINT "ingestion_tasks_ingestion_run_id_unique" UNIQUE("ingestion_run_id")
);
--> statement-breakpoint
...
ALTER TABLE "ai"."canvas_raw_documents" ADD CONSTRAINT "canvas_raw_documents_canvas_course_id_courses_id_fk" FOREIGN KEY ("canvas_course_id") REFERENCES "ai"."courses"("id") ON DELETE no action ON UPDATE no action;--> statement-breakpoint
...
ALTER TABLE "ai"."course_info" ADD CONSTRAINT "course_info_canvas_course_id_courses_id_fk" FOREIGN KEY ("canvas_course_id") REFERENCES "ai"."courses"("id") ON DELETE no action ON UPDATE no action;--> statement-breakpoint
...
ALTER TABLE "ai"."ingestion_tasks" ADD CONSTRAINT "ingestion_tasks_school_id_ingestion_runs_school_id_fk" FOREIGN KEY ("school_id") REFERENCES "ai"."ingestion_runs"("school_id") ON DELETE no action ON UPDATE no action;--> statement-breakpoint
