...
## Lifecycle of a request
    ...
    2.5 Upsert any courses that do not exist yet with their id and null to prevent any FK constraint errors.
    3. Compare the enrollments and update the user enrollments table as needed.
    3.5 Enrollment fork: See cases.
    4. Fetch the users activity stream from canvas
    5. Compare the latest stream id from local db to the canvas.
    6. If nothing has changed, return.
    7. If new items, pull the full stream, and start the logic for checking the stale data.
    8. Parse each stream item and enqueue an job to fetch, extract, load, and vectorize until the streamid equals the one in the db (see case 4).
